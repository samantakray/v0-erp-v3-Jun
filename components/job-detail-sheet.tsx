...
import { ArrowLeft, Calendar, CheckCircle2, Factory, Package, AlertTriangle } from "lucide-react"
...
import StoneSelectionViewComponent from "@/components/StoneSelectionView"
...
export function JobDetailSheet({ job, open, onOpenChange }) {
  const [currentPhase, setCurrentPhase] = useState(PHASES.STONE)
  const [jobStatus, setJobStatus] = useState(job?.status || "New Job")
  ...
  useEffect(() => {
    if (job) {
      switch (job.status) {
        case "New Job":
          setCurrentPhase(PHASES.STONE)
          break
        case "Stone Selected":
          setCurrentPhase(PHASES.DIAMOND)
          break
        case "Diamond Selected":
          setCurrentPhase(PHASES.MANUFACTURER)
          break
        case "In Production":
        case "Sent to Manufacturer":
          setCurrentPhase(PHASES.QC)
          break
        case "Quality Check":
        case "Completed":
  ...
  const getSteps = (): Step[] => {
    const phases = [
      { id: PHASES.STONE, label: "Stone" },
      { id: PHASES.DIAMOND, label: "Diamond" },
      { id: PHASES.MANUFACTURER, label: "Manufacturer" },
      { id: PHASES.QC, label: "QC" },
      { id: PHASES.COMPLETE, label: "Complete" },
    ]
    ...
    return phases.map((phase) => {
      if (phase.id === currentPhase) {
        ...
        (currentPhase === PHASES.DIAMOND && phase.id === PHASES.STONE) ||
        (currentPhase === PHASES.MANUFACTURER && (phase.id === PHASES.STONE || phase.id === PHASES.DIAMOND)) ||
        (currentPhase === PHASES.QC &&
          (phase.id === PHASES.STONE || phase.id === PHASES.DIAMOND || phase.id === PHASES.MANUFACTURER)) ||
  ...
  const handleCompleteStoneSelection = () => {
  ...
  const handleCompleteDiamondSelection = () => {
    if (
  ...
  const handleAssignManufacturer = () => {
    ...
    // In a real app, this would be an API call
    setJobStatus("Sent to Manufacturer")
  ...
  const handleCompleteQC = (passed: boolean) => {
    ...
    // In a real app, this would be an API call
    setJobStatus(passed ? "Quality Check Passed" : "Quality Check Failed")
    setStickerData({
  ...
  // If job status is "stone", show the dedicated Stone Selection View
  if (job.status === "stone") {
    return (
      <div className="fixed inset-0 z-50 bg-background">
        <div className="flex flex-col h-full">
          ...
          <div className="flex-1 p-6 overflow-y-auto">
            <StoneSelectionViewComponent
              job={job}
              onClose={() => {
                // This would typically refresh the job data
        ...
        <div className="flex flex-col h-full">
          <header className="flex h-14 lg:h-[60px] items-center gap-4 border-b bg-muted/40 px-6 sticky top-0 z-10">
            ...
            <div className="ml-auto flex items-center gap-2">
              <Button size="sm" disabled={jobStatus === "Completed"}>
          ...
          <div className="flex-1 p-6 overflow-y-auto">
            <div className="grid gap-6">
              ...
              {/* Stone Selection View - Only shown when job.status is "stone" */}
              {job.status !== "stone" ? (
                <>
                  {/* Stepper UI */}
                  <div className="mb-4">
                    ...
                    <CardContent>
                      <Tabs value={currentPhase}>
                        <TabsList className="grid grid-cols-5 mb-8">
                          <TabsTrigger
                            value={PHASES.STONE}
                            onClick={() => handleStepClick(PHASES.STONE)}
                            disabled={getSteps().find((s) => s.id === PHASES.STONE)?.status === "upcoming"}
                          >
                            Stone Selection
                          </TabsTrigger>
                          <TabsTrigger
                            value={PHASES.DIAMOND}
                            onClick={() => handleStepClick(PHASES.DIAMOND)}
                            disabled={getSteps().find((s) => s.id === PHASES.DIAMOND)?.status === "upcoming"}
                          >
                            Diamond Selection
                          </TabsTrigger>
                          <TabsTrigger
                            value={PHASES.MANUFACTURER}
                            onClick={() => handleStepClick(PHASES.MANUFACTURER)}
                            disabled={getSteps().find((s) => s.id === PHASES.MANUFACTURER)?.status === "upcoming"}
                          >
                            Manufacturer
                          </TabsTrigger>
                          <TabsTrigger
                            value={PHASES.QC}
                            onClick={() => handleStepClick(PHASES.QC)}
                            disabled={getSteps().find((s) => s.id === PHASES.QC)?.status === "upcoming"}
                          >
                            Quality Check
                          </TabsTrigger>
                          <TabsTrigger
                            value={PHASES.COMPLETE}
                            onClick={() => handleStepClick(PHASES.COMPLETE)}
                            disabled={getSteps().find((s) => s.id === PHASES.COMPLETE)?.status === "upcoming"}
                          >
                            Complete
                          </TabsTrigger>
                        </TabsList>
                        ...
                        {/* Stone Selection Tab */}
                        <TabsContent value={PHASES.STONE} className="space-y-4">
                          <div className="grid grid-cols-2 gap-4">
                            <div className="space-y-2">
                              ...
                              <Select
                                ...
                                disabled={currentPhase !== PHASES.STONE}
                              >
                          ...
                          <Separator />
                          <div className="flex justify-end">
                            <Button onClick={handleCompleteStoneSelection} disabled={currentPhase !== PHASES.STONE}>
                        ...
                        <TabsContent value={PHASES.DIAMOND} className="space-y-4">
                          <div className="grid grid-cols-2 gap-4">
                            <div className="space-y-2">
                              ...
                              <Select
                                ...
                                disabled={currentPhase !== PHASES.DIAMOND}
                              >
                          ...
                          <Separator />
                          <div className="flex justify-end">
                            <Button onClick={handleCompleteDiamondSelection} disabled={currentPhase !== PHASES.DIAMOND}>
                        ...
                        {/* Manufacturer Tab */}
                        <TabsContent value={PHASES.MANUFACTURER} className="space-y-4">
                                    ...
                                    <TableRow key={m.id}>
                                      ...
                                      <TableCell>
                                        <Button
                                          ...
                                          size="sm"
                                          onClick={() =>
                                            setManufacturerData({ ...manufacturerData, manufacturerId: m.id })
                                          }
                                          disabled={currentPhase !== PHASES.MANUFACTURER}
                                          className={
                          ...
                          <Separator />
                          <div className="flex justify-end">
                            <Button onClick={handleAssignManufacturer} disabled={currentPhase !== PHASES.MANUFACTURER}>
                              Assign Manufacturer
                            </Button>
                          </div>
                        </TabsContent>
                        ...
                        {/* Quality Check Tab */}
                        <TabsContent value={PHASES.QC} className="space-y-4">
                          <div className="grid grid-cols-2 gap-4">
                            <div className="space-y-2">
                            ...
                            <div className="space-y-2">
                              ...
                              <Input
                                ...
                                disabled={currentPhase !== PHASES.QC}
                              />
                            </div>
                        ...
                        {/* Complete Tab */}
                        <TabsContent value={PHASES.COMPLETE} className="space-y-4">
                          <div className="space-y-4">
                            <div className="grid grid-cols-2 gap-4">
                              <div>
                                ...
                                <div className="text-sm text-muted-foreground">
                                  {qcData.measuredWeight ? (
                                    <>
                                      <p>
                                        Result:{" "}
                                        {qcData.passed === null ? "Pending" : qcData.passed ? "PASSED" : "FAILED"}
                                      </p>
                          ...
                          <div className="flex justify-end">
                            <Button
                              ...
                              disabled={currentPhase !== PHASES.COMPLETE || jobStatus === "Completed"}
                            >
                              <CheckCircle2 className="mr-2 h-4 w-4" />
