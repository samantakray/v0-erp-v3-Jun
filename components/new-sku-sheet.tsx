...
export function NewSKUSheet({ open, onOpenChange, onSKUCreated = () => {} }) {
  ...
  // Generate a random 5-digit SKU number when the sheet is opened
  useEffect(() => {
    if (open) {
  ...
  const handleCreateSKU = () => {
    if (!createMultiple) {
      // For single SKU creation, use the image from the first SKU variant
      const firstSku = multipleSkus[0]
      const newSKU = {
        id: generateSKU(firstSku.category, skuNumber, firstSku.goldType, firstSku.stoneType),
  ...
  const handleConfirmMultiple = () => {
    ...
    if (createMultiple && multipleSkus.length > 0) {
      multipleSkus.forEach((sku) => {
        const skuId = generateSKU(sku.category, skuNumber, sku.goldType, sku.stoneType)
        const newSKU = {
          id: skuId,
      ...
      const firstSku = multipleSkus[0]
      const skuId = generateSKU(firstSku.category, skuNumber, firstSku.goldType, firstSku.stoneType)
      const newSKU = {
        id: skuId,
        name: `${firstSku.goldType} ${firstSku.category}${firstSku.stoneType !== "None" ? ` with ${firstSku.stoneType}` : ""}`,
        category: firstSku.category,
